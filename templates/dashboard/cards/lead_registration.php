...
if( $is_done && $is_done >= 10 ) {
  wp_redirect( apply_filters( 'futurewordpress/project/user/dashboardpermalink', $userInfo->ID, $userInfo->data->user_nicename ) );exit;
}
$needToSelect = false;
...
if( $regLink && ! empty( $regLink ) && ! $needToSelect ) {
  ...
  if( ! empty( $defaultContract ) ) {
    ...
    wp_die( __( 'Regisatration link not found.', 'stock-photo-duplicator' ) );
  }
}
...
if( is_wp_error( $userInfo ) || $errorHappens ) :
  ...
  add_filter( 'pre_get_document_title', function( $title ) {
    $title = apply_filters( 'futurewordpress/project/system/getoption', 'dashboard-title', __( 'Registration Field', 'stock-photo-duplicator' ) );
    return $title;
  }, 10, 1 );
  get_header();
  ?>
...
<?php if( true ) : ?>
  <?php include STOCK_PHOTO_DUPLICATOR_DIR_PATH . '/templates/dashboard/cards/select_registration_page.php'; ?>
<?php else: ?>
          ...
          <div class="content fs-6 d-flex flex-column-fluid">
            <div class="container-xxl">
              <div class="row g-5 g-xxl-12">
                <div class="col-xl-12">
                  <?php // include STOCK_PHOTO_DUPLICATOR_DIR_PATH . '/templates/dashboard/cards/form-wizard.php'; ?>
                </div>
              </div>
            </div>
          </div>
