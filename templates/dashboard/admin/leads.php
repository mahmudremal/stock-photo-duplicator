        ...
        <div class="card-body">
          <div class="fancy-table table-responsive border rounded">
            <table class="table table-striped mb-0">
              <thead>
                <tr>
                  <th scope="col"><?php esc_html_e( 'Email', 'stock-photo-duplicator' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Profiles', 'stock-photo-duplicator' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Client Status', 'stock-photo-duplicator' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Meeting Time & link', 'stock-photo-duplicator' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Retainer Amount', 'stock-photo-duplicator' ); ?></th>
                  <!-- Contract Options -->
                  <th scope="col"><?php esc_html_e( 'Status', 'stock-photo-duplicator' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Action', 'stock-photo-duplicator' ); ?></th>
                </tr>
              </thead>
              <tbody>
                <?php if( count( $subscribers ) <= 0 ) : ?>
                  <tr>
                    <td colspan="7"><img src="<?php echo esc_url( STOCK_PHOTO_DUPLICATOR_BUILD_URI . '/icons/not-found.svg' ); ?>" alt="<?php esc_attr_e( 'Noting found', 'stock-photo-duplicator' ); ?>" style="width: 100%;height: auto;max-height: 400px;"></td>
                  </tr>
                <?php endif; ?>
                <?php foreach( $subscribers as $subscriber ) :
                  ...
                  $doc = apply_filters( 'futurewordpress/project/esign/userdocument', false, $userInfo );$doc = ( $doc && is_array( $doc ) ) ? (object) $doc: $doc;
                  if( $doc && ! empty( $doc->document_status ) ) {$is_signed = ( $doc->document_status == 'signed' );} else {$is_signed = null;}
                    //   Missing
                  $is_signed = ( $is_signed === null ) ? __( 'Pending', 'stock-photo-duplicator' ) : (
                    ( $is_signed === true ) ? __( 'Signed', 'stock-photo-duplicator' ) : __( 'Pending', 'stock-photo-duplicator' )
                  );
                  $is_active = $activeSubscription ? __( 'Active', 'stock-photo-duplicator' ) : __( 'Paused', 'stock-photo-duplicator' );
                    ...
                    <td>
                      <div class="d-flex align-items-center">
                        <div class="media-support-info">
                          ...
                          <a class="mb-0" href="<?php echo esc_url( $userInfo->meta->meeting_link ); ?>" target="_blank">@<?php esc_html_e( 'Meeting link', 'stock-photo-duplicator' ); ?></a>
                          <?php endif; ?>
                        </div>
                      </div>
                    </td>
